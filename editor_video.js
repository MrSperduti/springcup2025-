document.addEventListener("DOMContentLoaded", function() {
  ...
  function renderTable(files) {
    ...
    files.forEach(file => {
      ...
      removeButton.onclick = function() {
        ...
        localStorage.setItem('videoFiles', JSON.stringify(updatedFiles));
  ...
  uploadFileButton.addEventListener('click', function() {
    ...
    if (files.length > 0) {
      let existingFiles = [];
      if (localStorage.getItem('videoFiles')) {
        existingFiles = JSON.parse(localStorage.getItem('videoFiles'));
      }
      ...
      localStorage.setItem('videoFiles', JSON.stringify(existingFiles));
      renderTable(existingFiles);

      previewDati.textContent = `Caricati ${files.length} video.`;
    } else {
  ...
  generateJsonButton.addEventListener('click', function() {
    ...
    const jsonBlob = new Blob([JSON.stringify({videoFiles: JSON.parse(localStorage.getItem('videoFiles'))})], {type: 'application/json'});
  ...
  if (localStorage.getItem('videoFiles')) {
    renderTable(JSON.parse(localStorage.getItem('videoFiles')));
  }
});
